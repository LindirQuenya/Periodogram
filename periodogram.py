...
#and nsamp*qmax*nbins*nbins times
    ...
    def blsL3(b,nb,bw,bm):
        bm[b]=bm[b-nb]
        bw[b]=bw[b-nb]

    def blsL4a(k,bw,bm,bc,sw,sm):
        bc[0]+=1
        sw[0]+=bw[k]
        sm[0]+=bm[k]
...
def readDoubleColumns(fname,arrayNames,delim):
    ...
    if arrayNames[DATA_FIELD_TYPE.DATA_X]==None:
        ...
        #the help message.
        indMap=[DATA_FIELD_TYPE.DATA_X,DATA_FIELD_TYPE.DATA_Y,DATA_FIELD_TYPE.DATA_Y_UNCERTAINTY,DATA_FIELD_TYPE.DATA_CONSTRAINT]
...
def computePeriodogram(args,data,fargs):
    ...
    if fargs.timeEst>0 and PRINT_TIMES:
        print("Estimated time for processing "+str(fargs.nsamp)+" periods: "+format(fargs.timeEst,'.4f')+' seconds ('+format(fargs.timeEst/60,'.4f')+' minutes)')
...
class pgramArgs:
    ...
    def populate(this):
        nbSet=0;qminSet=0;qmaxSet=0;nProcSet=0
        noutSet=0;bsmSet=0;stepSet=0;dfSet=0;
        ...
        for a in optList:
            if a[0]=='-a':
                ...
                if a[1] not in ['ls','bls','plav']:
                    raise ValueError("Inappropriate value for option -a:"+a[1]+"\nargument must be one of ls, bls, or plav!")
                ...
                if a[1] not in ["exp","std",'plav','fixedf','fixedp']:
                    raise ValueError("Inappropriate value for option -i:"+a[1]+"\nargument must be one of exp, std, plav, or fixedf!")
                #set pstepType
                this.pstepType=a[1]
            elif a[0]=='-I':
                #Implement later?
...
class funcArgs:
    ...
    def populate(this,args,data):
        ...
        if minperiod>maxperiod or minperiod<=0 or maxperiod<=0:
            if args.asFreq:
                raise ValueError("FrequencyRangeMin: frequency range error: from "+str(1/maxperiod)+" to "+str(1/minperiod))
            else:
        ...
        if args.periodFile==None:
            ...
            if ptype=="std":
                ...
                for i in range(nsamp):
                    ...
                    if (period[i]>(maxperiod+dell)) or\
                       (period[i]<(minperiod-dell)):
                        raise RuntimeError("PeriodStepMethod: bad period in fixedf: "+str(period[i])+" ("+str(minperiod)+" to "+str(maxperiod)+")")
                    f-=df
                ...
                for i in range(nsamp):
                    ...
                    if (period[i]>maxperiod+dell) or\
                       (period[i]<minperiod-dell):
                        raise RuntimeError("PeriodStepMethod: bad period in fixedp: "+str(period[i])+" ("+str(minperiod)+" to "+str(maxperiod)+")")
                    p+=dp
            ...
            if nsamp==0:
                raise ValueError("PeriodRangeMin: No periods satisfy min/max period constraints ("+str(minperiod)+"-"+str(maxperiod)+") with this PeriodStepMethod!")
        else:
